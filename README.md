...
5. 工具审批：**变更前确认**（build）/ **自动编辑**（edit）模式下，zcode 执行 Write/Edit/Bash 等工具前会弹窗，允许或拒绝；**计划模式**（plan）先出计划再动手；**完全访问**（yolo）减少确认（谨慎）。模式名与 ZCode 桌面端一致，悬停下拉项可见一句说明。
...
9. 工具栏状态旁显示 **上下文占用**（如 `上下文 14k/1.0M`，悬停看精确值与百分比，占用 ≥80% 变红）；每轮对话结束后自动刷新。
10. 工具栏自适应宽度：放不下时按钮/标签/下拉像文字一样整体流式换行、每行铺满，任何宽度都不丢按钮、不留大片空白。
