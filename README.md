...
在 IDEA 里与 [ZCode](https://zai.love/) AI 编码助手对话的插件：右侧 ToolWindow 聊天、选区/图片作为上下文、模型与思考强度切换、上下文占用显示、工具调用弹窗审批、修改的文件一键 diff、历史会话恢复与删除。
...
IDEA 插件（Kotlin）
 ...
 ├─ ZcodeSessionService —— 会话/进程管理，事件流翻译，消息排队
 ...
 ├─ SlashCommands —— "/" 命令：本地/服务端内置/自定义(.zcode/commands) 扫描、展开与路由
 ├─ SelectionContext —— 发送时读取编辑器选区/活动文件，注入 prompt；@引用记号展开
 └─ VFS 刷新 + DiffManager —— 感知并展示 zcode 对磁盘文件的修改
```

职责边界：代码搜索/阅读/修改由 zcode runtime 自己完成（ripgrep + Read/Edit/Write + 其自带 LSP 插件），直接操作磁盘文件；插件负责 IDE 上下文注入、审批交互、变更感知与展示，并使用 IDEA 索引为回答中的代码引用提供导航。协议细节见 [docs/PROTOCOL.md](docs/PROTOCOL.md)。
...
3. 编辑器中选中代码 → 右键 → **ZCode**：
   - **引用选中代码到对话...**：在输入框**光标处**插入引用记号（如 `@Foo.java:141-160`，对齐 zcode 的内嵌引用）——**可引用多处、可放在输入文字的任意位置**，记号就是普通文本（可编辑/删除）；发送时才展开成完整引用代码块，气泡回显保持紧凑记号。想移除某处引用，直接删掉对应记号即可；
   - 解释 / 优化 / 写测试 / 自定义指令：直接发送模板指令。
4. **图片输入**（当前模型支持时）：输入框 **Ctrl+V 粘贴剪贴板图片**，或点「附图」按钮选文件（可多张），缩略图 chip 展示、可移除，随下一条消息发送。附图入口按当前模型是否多模态自动启停。
5. **斜杠命令**：输入 `/` 弹出命令补全（↑↓ 选择，Enter/Tab 补全，Esc 关闭）。支持：
   - 插件本地命令：`/new`、`/clear`（新会话）、`/help`（命令列表）；
   - 服务端内置命令（goal/compact/init/plan/fork，随连接自动加载）；
   - **自定义命令**：放在 `~/.zcode/commands/*.md`（用户级）或项目 `.zcode/commands/*.md`（含上级目录，用户级优先）。Markdown 正文即命令内容，frontmatter 可写 `description`/`argument-hint`，正文用 `$ARGUMENTS`、`$1`..`$9` 引用参数（与 zcode CLI 一致）。
   - 未知命令会拒发并提示，不会把 `/xxx` 字面发给模型。
6. **手动压缩上下文**：对话多了上下文占用高时，点工具栏「压缩」按钮或直接点上下文标签（等同发送 `/compact`），zcode 会总结当前对话释放空间；当前有轮次运行时会自动排队，压缩结束后"上下文 x/y"明显下降。
7. 工具审批：**变更前确认**（build）/ **自动编辑**（edit）模式下，zcode 执行 Write/Edit/Bash 等工具前会弹窗，允许或拒绝；**计划模式**（plan）先出计划再动手；**完全访问**（yolo）减少确认（谨慎）。模式名与 ZCode 桌面端一致，悬停下拉项可见一句说明。
8. **变更文件 (N)** 按钮列出本会话被 zcode 修改过的文件，点击用 IDE 原生 diff 对比修改前内容。
9. **新会话 / 恢复…**：会话持久化在 zcode 侧（`~/.zcode/cli/`），可跨 IDE/CLI 恢复；恢复列表每行带 🗑 删除按钮（确认后从 zcode 会话库删除该会话及其全部消息）。
10. 工具栏右侧可切换 **模型** 与 **思考强度**（低/高/最高，随模型变化）：切换立即作用于当前会话并被记住，之后新建/恢复的会话沿用。
11. 工具栏状态旁显示 **上下文占用**（如 `上下文 14k/1.0M`，悬停看精确值与百分比，占用 ≥80% 变红并建议压缩）；每轮对话结束后自动刷新。
12. 工具栏自适应宽度：放不下时按钮/标签/下拉像文字一样整体流式换行、每行铺满，任何宽度都不丢按钮、不留大片空白。
13. **回答阅读体验**（对齐 zcode）：思考过程与工具调用（Read/Bash/Edit…）按发生顺序收进顶部可折叠的时间线（默认折叠，标题实时显示"思考中 · N 字 · N 个工具"进度，回答完成后定格为"已深度思考 · N 字 · N 个工具"，点击可展开回看）；工具行点击直接打开对应文件；**正文结论始终在最后**。恢复历史会话的结构与实时一致。流式输出自动贴底跟随，上翻阅读历史则停在原地不被拽回，拉回底部自动恢复。
14. **代码引用可点击跳转**：文件引用支持单行和行范围（如 `Foo.java:336-347`、`Foo.java#L12-L20`），点击可定位并选中范围；同名文件弹出路径列表供选择。回答完成后，在后台通过 IDE 索引解析行内类名、方法名等符号，仅为找到定义的内容添加链接，多个定义可选择。可跳转内容显示下划线和悬停提示，配置值等普通代码文本保持原样。
15. **Markdown 完整渲染**：表格按 GFM 渲染成 GitHub 风格表格（横向细线分隔 + 表头底色加粗 + 斑马纹，支持列对齐，不再是管道源码）；围栏代码块用 **IDE 自己的语法高亮**着色（与编辑器同款配色、自动跟随主题）；另支持任务列表 ☐/☑、删除线等。
...
- **完全访问（yolo）模式**：工具全自动执行（包括 shell），仅在可信项目使用。
...
- 变更文件实时列表、嵌套列表渲染、`session/fork`/rewind UI
